...
/obj/gta13cars
	...
	layer = 3.8 + 0.1 //so it sits above objects including mobs
	density = 1
	anchored = 1
	animate_movement=1
//	light_power = 1.5
...
/obj/gta13cars/Move()
	...
	if(world.time > l_move_time + move_delay)
		...
		if(load && !istype(load, /datum/gta13cars_dummy_load))
			load.forceMove(loc)
			load.set_dir(dir)
		if(passenger)
...
/obj/gta13cars/attackby(obj/item/weapon/W as obj, mob/user as mob)
...
/obj/gta13cars/bullet_act(var/obj/item/projectile/Proj)
	health -= Proj.damage
	..()
	healthcheck()

/obj/gta13cars/ex_act(severity)
	switch(severity)
		if(1.0)
			explode()
			return
...
/obj/gta13cars/emp_act(severity)
	var/was_on = on
	stat |= EMPED
...
/obj/gta13cars/emag_act(mob/user)
	if(locked)
		locked = 0
...
/obj/gta13cars/attack_ai(mob/user as mob)
	return

// For downstream compatibility (in particular Paradise)
/obj/gta13cars/proc/handle_rotation()
	return

//-------------------------------------------
// Vehicle procs
//-------------------------------------------
/obj/gta13cars/proc/turn_on()
	if(stat)
		return 0
	...
	verbs -= /obj/gta13cars/verb/stop_engine
	verbs -= /obj/gta13cars/verb/start_engine
...
/obj/gta13cars/proc/headlights_on()

	headlights = 1
	set_light(3)

	verbs -= /obj/gta13cars/verb/turn_headlights_off
	verbs -= /obj/gta13cars/verb/turn_headlights_on
...
/obj/gta13cars/proc/headlights_off()
...
/obj/gta13cars/proc/swap()
	return

/obj/gta13cars/proc/honk_horn()
	playsound(src, 'sound/items/bikehorn.ogg',40,1)

/obj/gta13cars/proc/explode()
...
/obj/gta13cars/proc/healthcheck()
	if(health <= 0)
		explode()

/obj/gta13cars/proc/powercheck()
	if(!cell && !powered)
		return
...
/obj/gta13cars/proc/insert_cell(var/obj/item/weapon/cell/cell/C, var/mob/living/carbon/human/H)
	if(cell)
		return
	if(!istype(cell))
		return

	H.drop_from_inventory(cell)
	cell.forceMove(src)
	powercheck()
	usr << "<span class='notice'>You install [cell] in [src].</span>"

/obj/gta13cars/proc/remove_cell(var/mob/living/carbon/human/H)
	if(!cell)
		return
...
/obj/gta13cars/proc/RunOver(var/mob/living/carbon/human/H)
...
//-------------------------------------------
/obj/gta13cars/proc/load(var/atom/movable/C)
...
/obj/gta13cars/proc/unload(var/mob/user, var/direction)
	if(!load)
		return
...
/obj/gta13cars/verb/turn_headlights_on()
...
/obj/gta13cars/verb/turn_headlights_off()
...
/obj/gta13cars/verb/start_engine()
...
/obj/gta13cars/verb/stop_engine()
...
/obj/gta13cars/verb/open_trunk()
	set name = "Trunk Open"
	set category = "Vehicle"
	set src in oview(1)
	return
/obj/gta13cars/verb/close_trunk()
	set name = "Trunk Close"
	set category = "Vehicle"
	set src in oview(1)
	return
/obj/gta13cars/verb/honk()
	set name = "Honk horn"
	set category = "Vehicle"
	set src in view(0)
...
// Stat update procs
//-------------------------------------------------------
/obj/gta13cars/proc/update_stats()
	return

/obj/gta13cars/attack_generic(var/mob/user, var/damage, var/attack_message)
	if(!damage)
		return
...
/obj/gta13cars/proc/populate_action_buttons() //These come with all vehicles
...
/obj/gta13cars/proc/populate_verbs()
	verbs -= /obj/gta13cars/verb/close_trunk
	verbs -= /obj/gta13cars/verb/open_trunk
	verbs -= /obj/gta13cars/verb/stop_engine
	verbs -= /obj/gta13cars/verb/turn_headlights_off