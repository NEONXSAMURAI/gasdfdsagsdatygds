...
/obj/gta13cars/car/Move(var/turf/destination)
...
/obj/gta13cars/car/attackby(obj/item/weapon/W as obj, mob/user as mob)
	if(istype(W, /obj/item/weapon/key/car))
		if(!key)
			user.drop_item()
			W.forceMove(src)
			key = W
			verbs += /obj/gta13cars/car/verb/remove_key
		return
	..()

/obj/gta13cars/car/Bump(atom/Obstacle)
	if(!emagged)
		if(!bumped)
	...
	if(istype(A, /obj/gta13cars/car/))
		var/obj/gta13cars/car/C = A
		var/mob/living/D = load
	...
	if(istype(A, /obj/structure))
		...
		if(istype(A, /obj/structure/table/glass))
			var/obj/structure/table/glass/G = A
			G.shatter()
...
/obj/gta13cars/car/proc/take_damage(damage)
	src.health -= damage
	if(prob(10))
...
/obj/gta13cars/car/proc/container_resist(var/mob/living/escapee)
	...
	to_chat(escapee, "<span class='warning'>You lean on the back of \the trunk and start pushing the door open. (this will take about [breakout_time] minutes)</span>")
...
/obj/gta13cars/car/turn_off()
	..()

/obj/gta13cars/car/proc/trunk_closed()

	trunk_open = 0

	verbs -= /obj/gta13cars/car/close_trunk
	verbs -= /obj/gta13cars/car/open_trunk
	if(!trunk_open)
		verbs += /obj/gta13cars/car/open_trunk
	else
		verbs += /obj/gta13cars/car/close_trunk
...
/obj/gta13cars/car/proc/trunk_opened()
...
/obj/gta13cars/car/headlights_on()
	if(!key)
		return
	else
		..()

/obj/gta13cars/car/proc/stop_short() //whiplash!
	var/pixel_x_diff = 0
	var/pixel_y_diff = 0
	switch(dir)
		if(NORTH)
...
/obj/gta13cars/car/relaymove(mob/user, direction)
	if(user != load)
		return 0
...
/obj/gta13cars/car/examine(mob/user)
	if(!..(user, 1))
		return
...
/obj/gta13cars/car/start_engine()
...
/obj/gta13cars/car/verb/remove_key()
	set name = "Remove key"
	set category = "Vehicle"
	set src in view(0)
...
/obj/gta13cars/car/load(var/atom/movable/C, who)
	if(C == src)
		return 0
...
/obj/gta13cars/car/unload(var/mob/user, who, var/direction)
	switch(who)
		if("driver")
			if(!load) return
		if("passenger")
...
/obj/gta13cars/car/populate_action_buttons()
...
/obj/gta13cars/car/populate_verbs()
	..()
	verbs -= /obj/gta13cars/car/stop_engine
	verbs -= /obj/gta13cars/car/start_engine
	verbs -= /obj/gta13cars/car/close_trunk

/obj/gta13cars/car/proc/update_dir_car_overlays()
	return