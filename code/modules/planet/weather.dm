...
/datum/weather_holder/proc/update_icon_effects()
	if(current_weather)
		for(var/turf/simulated/floor/T)
			...
			T.weather_overlay = image(icon = current_weather.icon, icon_state = current_weather.icon_state, layer = ABOVE_TILE_LAYER)
			T.overlays += T.weather_overlay
...
/datum/weather_holder/sif
	...
	planetary_wall_type = /turf/unsimulated/wall
